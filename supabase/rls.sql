...
-- ═══════════════════════════════════════════════
-- TABLE: notifications
-- User can read their own notifications and create notifications
-- ═══════════════════════════════════════════════

ALTER TABLE public.notifications ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "user_read_notifications"   ON public.notifications;
DROP POLICY IF EXISTS "user_create_notifications" ON public.notifications;
...
CREATE POLICY "user_create_notifications"
  ON public.notifications
  FOR INSERT
...
-- ═══════════════════════════════════════════════
-- TABLE: user_settings
-- User can read/update their own settings only
-- ═══════════════════════════════════════════════

ALTER TABLE public.user_settings ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "user_read_settings"   ON public.user_settings;
DROP POLICY IF EXISTS "user_upsert_settings" ON public.user_settings;

CREATE POLICY "user_read_settings"
  ON public.user_settings
  FOR SELECT
  USING ( user_id = private.current_user_id() );

CREATE POLICY "user_upsert_settings"
  ON public.user_settings
  FOR ALL
  USING ( user_id = private.current_user_id() )
  WITH CHECK ( user_id = private.current_user_id() );
