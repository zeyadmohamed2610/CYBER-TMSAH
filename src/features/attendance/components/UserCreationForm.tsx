...
export const UserCreationForm = () => {
    ...
    supabase.from("subjects").select("id, name, doctor_name").order("name")
      .then(({ data }) => {
        if (data) {
          ...
          if (data.length > 0) {
            setSSubject(data[0].id);
            setDSubject(data[0].id);
          }
        }
      });
  }, []);
  ...
  const handleStudent = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await userService.createUser({
      ...
      role: "student", subjectId: sSubject || null,
    });
    if (res.error) toast({ variant: "destructive", title: "فشل الإنشاء", description: res.error });
    else {
      toast({ title: "تم إنشاء الطالب ✓", description: `تم إضافة ${sName} بنجاح.` });
  ...
  const handleDoctor = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (res.error) toast({ variant: "destructive", title: "فشل الإنشاء", description: res.error });
      ...
      toast({ title: "تم إنشاء الدكتور ✓", description: `تم إضافة ${dName} وربطه بمادته.` });
      setDName(""); setDEmail(""); setDPass("");
      // Refresh subjects to show updated doctor name
      supabase.from("subjects").select("id, name, doctor_name").order("name")
        ...
        <Tabs defaultValue="student">
          ...
          <TabsContent value="student">
            <form onSubmit={handleStudent} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={sSubject} onValueChange={setSSubject} disabled={submitting}>
                  <SelectTrigger><SelectValue placeholder="اختر المادة" /></SelectTrigger>
                  <SelectContent>{subjectOptions}</SelectContent>
                </Select>
              </div>
          ...
          <TabsContent value="doctor">
            <form onSubmit={handleDoctor} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {subjects.length === 0 ? (
                  <p className="text-xs text-muted-foreground flex items-center gap-1">
                    ...
                    لا توجد مواد — أنشئ مادة أولاً من تبويب المواد
                  </p>
                ) : (
              ...
              <Button className="w-full" type="submit" disabled={submitting || !dName || !dEmail || !dPass || !dSubject}>
